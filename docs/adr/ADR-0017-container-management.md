...
Adopt **Portainer CE** as the central container management and control plane, running on a dedicated **ctrl** node, with Portainer agents deployed on all Docker hosts (dns, svcs, ai, obs, TrueNAS host). **The DMZ node is explicitly excluded** — no Portainer agent (including Edge Agent) runs on DMZ workloads, per ADR-0021.
...
* ~~The Portainer agent for the DMZ node will be deployed inside the relevant VM~~ — **superseded by ADR-0021**: the DMZ services VM runs no Portainer agent; management is Docker CLI via Tailscale SSH only
...
**Implication for this ADR:** The DMZ node is outside the Portainer control plane (ADR-0021). DMZ container workloads are managed via Docker CLI over Tailscale SSH.

## Follow-ups

* ~~Revisit this ADR after DMZ node is operational~~ — DMZ node (hp600a) is now running Proxmox VE. ADR updated accordingly.
* Deploy DMZ services VM on hp600a (no Portainer agent — ADR-0021)
