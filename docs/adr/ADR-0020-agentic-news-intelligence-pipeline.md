...
> **Amendment (2026-07-11) — serving backends replaced, per ADR-0024:**
> - The iGPU lane no longer runs Ollama. `{{OLLAMA_AMD_HOSTNAME}}` is retired.
>   ADR-0024 defines the replacement: primary path Lemonade Server (NPU+iGPU
>   hybrid), fallback dual vLLM instances (`llm-a-n5.{{MY_DOMAIN}}` for the large
>   model, `vllm-b-n5.{{MY_DOMAIN}}` for the smaller shared model). This
>   pipeline's large-model calls (deep analysis, briefing) map to the large-model
>   endpoint of whichever path wins.
> - §3 Hermes/n8n config: `base_url` must point at the new endpoint's `/v1`. All
>   backends are OpenAI-compatible — endpoint URL change only, no payload/format
>   changes.
> - The `OLLAMA_CONTEXT_LENGTH` server-side note in §3 is obsolete. Context
>   length is now set server-side per container (e.g. vLLM `--max-model-len`).
>   Minimum 16k–32k for agent tool schemas still applies.
> - The B50 lane also changes: `{{OLLAMA_B50_HOSTNAME}}` (llama.cpp/Vulkan) is
>   replaced by a vLLM endpoint with a ~14B model (name assigned at deployment).
>   Fast-lane triage calls in §3 map to it or to `vllm-b-n5`.
> - References to `{{OLLAMA_AMD_HOSTNAME}}:{{OLLAMA_AMD_PORT}}` / `{{OLLAMA_B50_HOSTNAME}}` in §2
>   (Tailscale allowlist), §3, and the architecture diagram should be read
>   against the new endpoints; the karakuri Tailscale ACL must target the new
>   names/ports when implemented.
...
### 2. Agent Host: {{KARAKURI_HOSTNAME}} (isolated VM — placement **TBD**)

> **Placement status (2026-06-10):** Not yet implemented. The DMZ-box placement below is a candidate, not a decision. **If implemented on the DMZ box**, the Tailscale allowlist paths listed here constitute DMZ → Management traffic and MUST be recorded as an explicit, scoped exception to ADR-0016 default-deny *before* implementation. Alternative: host the VM on internal infrastructure, which requires no exception.
